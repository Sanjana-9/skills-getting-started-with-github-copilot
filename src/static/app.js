      ...
      Object.entries(activities).forEach(([name, details]) => {
        ...
        const participantsHtml = details.participants.length
          ? `<ul>${details.participants
              .map(
                (participant) =>
                  `<li><span class="participant-email">${participant}</span><button class="delete-participant" type="button" data-activity="${name}" data-email="${participant}" aria-label="Remove participant">&times;</button></li>`
              )
              .join("")}</ul>`
          : `<p class="empty-list">No participants have signed up yet.</p>`;
